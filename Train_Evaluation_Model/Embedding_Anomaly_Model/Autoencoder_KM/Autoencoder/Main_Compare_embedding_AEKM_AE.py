...
path_save_file= '/blue/salemi/share/varcovid/SECONDO_ANNO/Pipeline/AutoEncoder_KM/AIME/Official_simulation/Simulation/Autoencoder/' # where save the file
# columns in metadata
...
df_trainstep_1, train_w_list = load_data(dir_week, [54]) # First training set.
...
input_dim = 32  # Aggiusta in base alla dimensione dei tuoi dati
latent_dim = 16  # Dimensione dello spazio latente
noise_factor = 0.05  # Regola il livello di rumore
autoencoder_c = build_noisy_autoencoder(input_dim=32, latent_dim=latent_dim, noise_factor=noise_factor)
autoencoder_c.compile(optimizer='adam', loss='mean_squared_error')
autoencoder_c.fit(train, train, epochs=50, batch_size=512)

# Define the treshold
training_errors = compute_reconstruction_error(autoencoder_c,train)  # Assicurati che `x_train` sia il tuo dataset di training
threshold = define_anomaly_threshold(training_errors,0.98)
...
for combo in all_combo[0:1]:
    ...
    for week in range(n_training, 200):  # metadata['week'].max()
        ...
        new_data_error = compute_reconstruction_error(autoencoder_c, test_step_i)  # `new_data` è il vettore o batch di vettori da testare
...
df_conf.to_csv(path_save_file + 'conf_mat_over_time_ae.tsv', sep='\t', index=None)
