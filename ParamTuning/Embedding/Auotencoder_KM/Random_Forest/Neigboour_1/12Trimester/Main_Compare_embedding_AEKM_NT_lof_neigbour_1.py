...
path_save_file= '/blue/salemi/share/varcovid/SECONDO_ANNO/Pipeline/AutoEncoder_KM/AIME/Official_simulation/Neigboour_1/12Trimester/' # where save the file
# columns in metadata
...
for combo in all_combo[0:1]:
    ...
    for week in range(n_training, 54):  # metadata['week'].max()
        ...
        y_test_i_predict = clf_c.fit_predict(test_step_i)
...
df_conf.to_csv(path_save_file + 'conf_mat_over_time_if.tsv', sep='\t', index=None)

#PCA
lineages_train = np.array(transform_labels(lineages_train,valid_lineage_lineage))
plot_pca_with_centroids(train_completo,lineages_train)
plot_pca_with_centroids_zoomed(train_completo,lineages_train)

#Tsne
plot_tsne_with_centroids(train_completo,lineages_train)
plot_tsne_with_centroids_zoomed(train_completo,lineages_train)