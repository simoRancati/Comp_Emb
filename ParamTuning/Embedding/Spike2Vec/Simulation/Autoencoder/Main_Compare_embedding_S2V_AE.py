...
dir_week = '/blue/salemi/share/varcovid/SECONDO_ANNO/dataset_nov_2023_little_spike2vec_World/'  # path del Dataset #path del Dataset
...
path_save_file= '/blue/salemi/share/varcovid/SECONDO_ANNO/Pipeline/Spike2Vec/AIME/Official_simulation/Simulation/Autoencoder/' # where save the file
# columns in metadata
...
rbf_feature.fit(train_step1[selected_indices])
print('ho finito di fare s2v')
train= rbf_feature.transform(train_step1)
print('ho finito di trasformare')

# training autoencoders
input_dim = 500  # Aggiusta in base alla dimensione dei tuoi dati
latent_dim = 16  # Dimensione dello spazio latente
noise_factor = 0.05  # Regola il livello di rumore
autoencoder = build_noisy_autoencoder(input_dim=500, latent_dim=latent_dim, noise_factor=noise_factor)
...
test_step_tot = np.ones((1,500))
y_test_nparray_fc = []

# Built test set
train_completo = train.copy() # contain the sequence with all the kmers
for week in range(n_training-1, 200):  # metadata['week'].max()
    ...
    test_step_i = rbf_feature.transform(test_step_i)
...
df_conf.to_csv(path_save_file + 'conf_mat_over_time_s2v_ae.tsv', sep='\t', index=None)
