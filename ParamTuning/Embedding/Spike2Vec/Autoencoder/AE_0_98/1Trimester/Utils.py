...
def lineages_of_interest():
    valid_lineage_FDLs = ['B.1.1.284', 'B.1.177', 'B.1.2', 'B.1.1.7', 'AY.4', 'B.1.617.2', 'AY.29', 'AY.103','AY.43', 'AY.44', 'BA.2', 'BA.1', 'BA.2.9', 'BA.2.12.1', 'BA.5', 'BA.5.1', 'BA.5.2', 'BR.2.1', 'XBB.1.5','CH.1.1', 'FK.1.1', 'XBC.1.6', 'XBC.1.3', 'DV.7.1', 'HW.1.1', 'EG.5.1']
...
def hash2vec(seq_data,kmers,n_c):
    num_hash_fun = 1
    frequency_vector = []
    ...
    for seq_ind in range(len(seq_data)):
        ...
        index_dif_0 = np.where(se_temp > 0)
        kmers_list = list(kmers[index_dif_0])

        # create dictionary
        idx = pd.Index(kmers_list)
        aq = idx.value_counts()
    ...
    frequency_vector_dr = pca.fit_transform(frequency_vector)

    return np.array(frequency_vector_dr)
