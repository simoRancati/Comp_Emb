...
path_save_file= '/blue/salemi/share/varcovid/SECONDO_ANNO/Pipeline/Spike2Sig/AIME/Official_simulation/AE_0_98/123Trimester/' # where save the file
# columns in metadata
...
input_dim = min_length  # Aggiusta in base alla dimensione dei tuoi dati
latent_dim = 16  # Dimensione dello spazio latente
noise_factor = 0.05  # Regola il livello di rumore
autoencoder = build_noisy_autoencoder(input_dim=min_length, latent_dim=latent_dim, noise_factor=noise_factor)
...
df_conf.to_csv(path_save_file + 'conf_mat_over_time_s2s_ae.tsv', sep='\t', index=None)