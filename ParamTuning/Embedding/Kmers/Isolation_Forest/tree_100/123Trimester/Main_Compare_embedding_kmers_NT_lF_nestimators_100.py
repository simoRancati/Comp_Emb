...
path_save_file= '/blue/salemi/share/varcovid/SECONDO_ANNO/Pipeline/Kmers/AIME/Official_simulation/tree_100/123Trimester/' # where save the file
# columns in metadata
...
ind_scaling = 0
pca_class = np.ones(len(i_no_zero))
...
for week in range(n_training-1, 54):  # metadata['week'].max()
    ...
    test_step_completo = test_step_i[:, i_no_zero]
...
df_conf.to_csv(path_save_file + 'conf_mat_over_time_kmers_if.tsv', sep='\t', index=None)
